(function() {
	...
	function repositoryParticipant(breeze, abstractRepository, config, cart) {
		...
		function Ctor(mgr) {
			...
			this.getParticipantByHouseId = getParticipantByHouseId;
			this.getParticipantByEmailAddress = getParticipantByEmailAddress;
			this.getParticipantByGuid = getParticipantByGuid;
			this.getEmployeeByEmailAddress = getEmployeeByEmailAddress;
		...
		function getParticipantByHouseId(houseId) {
			var self = this;
			var query = entityQuery.from('Participants')
		...
		function getParticipantByGuid(partGuid) {
			var self = this;
			var query = entityQuery.from('Participants')
				.where('participantGuid', '==', partGuid);
		...
		function getEmployeeByEmailAddress(employeeEmail, ownerId) {
			var self = this;
			var pred = predicate.create('email', '==', employeeEmail)
				.and('ownerId', '==', ownerId);
